...
energydif = [-2.6255645259353013, -2.7384239947347595, -2.2381947822714636, -1.544642002594557,-1.0838811233819996,-0.6596345913862223,-0.6390709722185619,-0.6333040869282485]
decrement =6
for j in range(0,len(data)):
   ...
   # write('newimage.traj',atoms)
    ...
    fig.add_subplot(ax)
    if (j ==0):
       name2 ='Pt$_7$O$_y$CO Lowest Isomers'
       ax.set_title(name2)
    ...
    name ='$\Delta E_f = {:3.3f}$ eV'.format(energydif[j])
    ax.text(0.05, -0.14, name, transform=ax.transAxes,fontsize=8)
    name1 = "S$_{"+ str(j+1) + "}$"
...
#---------------------- Pt7CO2 attached form clusters -------------------------------------#
data=read(sys.argv[2]+'@:')
energydif =np.zeros(len(data))
energydif = [-2.7598235392131727,
-2.3989677622447942,
-1.8522156668483802,
-1.889517725350217,
 -1.6841562635844838,
...
decrement =6
for j in range(0,len(data)):
    inner = gridspec.GridSpecFromSubplotSpec(2, 1,subplot_spec=outer[j+9], wspace=0.00, hspace=0.0, height_ratios=[6.86,9.9])
    atoms = data[j]
    colorlenth = len(atoms)
    atoms =atoms*(3,3,1)
    print(colorlenth)
   ...
   # write('newimage.traj',atoms)
    ...
    fig.add_subplot(ax)
    if (j ==0):
       name2 ='Pt$_7$O$_{y-1}$CO2 attached Lowest Isomers'
       ax.set_title(name2)
...
#---------------------- Pt7CO2 detached form clusters -------------------------------------#
data=read(sys.argv[3]+'@:')
energydif =np.zeros(len(data))
energydif = [-1.8648004722300993,
-1.8689609216367415,
-1.7702248424551925,
-2.2616877616323308,
 -2.662659589052211,
...
decrement =6
for j in range(0,len(data)):
    inner = gridspec.GridSpecFromSubplotSpec(2, 1,subplot_spec=outer[j+18], wspace=0.00, hspace=0.0, height_ratios=[6.86,9.9])
    atoms = data[j]
    colorlenth = len(atoms)
    atoms =atoms*(3,3,1)
    print(colorlenth)
   ...
   # write('newimage.traj',atoms)
    ...
    fig.add_subplot(ax)
    if (j ==0):
       name2 ='Pt$_7$O$_{y-1}$CO2 detached Lowest Isomers'
       ax.set_title(name2)
...
#---------------------- Pt7CO3 form clusters -------------------------------------#
data=read(sys.argv[4]+'@:')
energydif =np.zeros(len(data))
energydif = [-1.8149340582595368,
-1.6336370949285737,
-1.3397380262315863,
-2.614886682910239,
 -3.173763379310614,
...
decrement =6
for j in range(0,len(data)):
    inner = gridspec.GridSpecFromSubplotSpec(2, 1,subplot_spec=outer[j+27], wspace=0.00, hspace=0.0, height_ratios=[6.86,9.9])
    atoms = data[j]
    colorlenth = len(atoms)
    atoms =atoms*(3,3,1)
    print(colorlenth)
   ...
   # write('newimage.traj',atoms)
    ...
    fig.add_subplot(ax)
    if (j ==0):
       name2 ='Pt$_7$O$_{y-2}$CO$_3$ Lowest Isomers'
       ax.set_title(name2)
