...
for j in range(0,8):
   ...
   # write('newimage.traj',atoms)
    a=atoms
    del atoms[[atom.index for atom in atoms if atom.index <=colorlenth*5-11 or atom.index >=colorlenth*5]]
    #view(atoms)
    ...
    if (j == 2):
       ...
       del atoms[atoms.positions[:,1] <= centreofmass[1]-9.10]
    ...
    if (j ==2):
       ax.set_ylim([centreofmass[1]-8.5, centreofmass[1]+5.0])
    ...
    fig.add_subplot(ax)
fig.text(0.5, 0.89, 'Lowest Isomers of Pt$_7$O$_{3}$', ha='center',fontsize=14)
name = sys.argv[2]
name =name
