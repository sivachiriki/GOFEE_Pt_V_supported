...
traj = Trajectory('Pt7O2_CO_Al2O3_KRRfund9l_DFTrelaxed1.traj','w')
for i in range(1,3):
    data =[]
    stru = '{}.traj'.format(i)
    data = read(stru+'@:')
...
data1=read('Pt7O2_CO_Al2O3_KRRfund9l_DFTrelaxed1.traj@:')
data1.sort(key=lambda x: x.get_potential_energy())
traj = Trajectory('Pt7O2_CO_Al2O3_KRRfund9l_DFTrelaxedsorted1.traj','w')
