...
#plt.title('Pt$_7$O$_1$ Lowest Isomers')
#plt.xticks([])
#plt.yticks([])
...
for j in range(0,len(data)):
   ...
   # write('newimage.traj',atoms)
    a=atoms
    del atoms[[atom.index for atom in atoms if atom.index <=colorlenth*5-9 or atom.index >=colorlenth*5]]
    #view(atoms)
...
#       ax.set_title(name2)
    ...
    if (j ==6):
       ax.set_xlim([centreofmass[0]-10.5, centreofmass[0]+4.50])
    ...
    fig.add_subplot(ax)
fig.text(0.4, 0.89, 'Lowest Isomers of Pt$_7$O$_{1}$', ha='center',fontsize=14)
name = sys.argv[2]
name =name
