...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 295 and i <=308 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 448 and i <=462 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 603 and i <=616 ):
           colors[i] =[0.1, 0.2, 0.2]
...
data=read('Pt7_6O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt7_6O2_Al2O3_0001_DFTopt_g{}'.format(j)
