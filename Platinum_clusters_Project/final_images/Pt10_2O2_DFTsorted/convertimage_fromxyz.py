...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 294 and i <=298 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 443 and i <=447 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 592 and i <=596 ):
           colors[i] =[0.1, 0.2, 0.2]
...
data=read('Pt10_2O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt10_2O2_Al2O3_0001_DFTopt_g{}'.format(j)
