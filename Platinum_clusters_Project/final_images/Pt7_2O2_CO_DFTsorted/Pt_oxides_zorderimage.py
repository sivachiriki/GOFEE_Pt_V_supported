...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 283 and i <=296 ):
           colors[i] =[153/255, 0/255, 0/255]
        if (atom.number ==8 and i >= 431 and i <=444 ):
           colors[i] =[153/255, 0/255, 0/255]
        if (atom.number ==8 and i >= 579 and i <=592 ):
           colors[i] =[153/255, 0/255, 0/255]
...
data=read('Pt7O4CO_Al2O3_KRRfund9l_DFTrelaxedsorted.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt7O4CO_Al2O3_0001_Echem_DFTopt_{}'.format(j)
