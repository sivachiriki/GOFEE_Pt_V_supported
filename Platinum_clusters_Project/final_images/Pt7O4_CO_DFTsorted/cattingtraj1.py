...
traj = Trajectory('Pt7O4_CO_Al2O3_KRRfund9l_DFTrelaxed1.traj','w')
for i in range(10,11):
    data =[]
    stru = '../Pt7_2O2_CO_{}/Pt7O4_CO_Al2O3_KRRfund9l_DFTrelaxed.traj'.format(i)
    data = read(stru+'@:')
    data.sort(key=lambda x: x.get_potential_energy())
    for j in range(0,len(data)):
        traj.write(data[j])

data1=read('Pt7O4_CO_Al2O3_KRRfund9l_DFTrelaxed1.traj@:')
data1.sort(key=lambda x: x.get_potential_energy())
traj = Trajectory('Pt7O4_CO_Al2O3_KRRfund9l_DFTrelaxedsorted1.traj','w')
