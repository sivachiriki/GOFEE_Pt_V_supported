...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 135 and i <=148 ):
           colors[i] =[0.1, 0.2, 0.5]
        if (atom.number ==8 and i >= 281 and i <=296 ):
           colors[i] =[0.1, 0.2, 0.4]
        if (atom.number ==8 and i >= 430 and i <=444 ):
           colors[i] =[0.1, 0.2, 0.4]
        if (atom.number ==8 and i >= 578 and i <=592 ):
           colors[i] =[0.1, 0.2, 0.4]
...
data=read('Pt7O4_CO_Al2O3_KRRfund9l_DFTrelaxedsorted1.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt7O4_CO_Al2O3_0001_DFTopt_n{}'.format(j)
