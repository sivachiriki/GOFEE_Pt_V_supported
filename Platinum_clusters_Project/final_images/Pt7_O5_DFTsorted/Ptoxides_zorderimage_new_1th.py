...
for j in range(1,len(data)):
   ...
   # write('newimage.traj',atoms)
    #exit()
    a=atoms
    del atoms[[atom.index for atom in atoms if atom.index <=colorlenth*5-13 or atom.index >=colorlenth*5]]
    #view(atoms)
    ...
    del atoms[atoms.positions[:,0] <= centreofmass[0]-9.0]
    del atoms[atoms.positions[:,1] >= centreofmass[1]+10.0]
    del atoms[atoms.positions[:,1] <= centreofmass[1]-5.5]
    ...
    ax.set_xlim([centreofmass[0]-8.0, centreofmass[0]+8.0])
    ax.set_ylim([centreofmass[1]-5.5+0.5, centreofmass[1]+10.0-1.50])
