...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 296 and i <=302 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 447 and i <=453 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 598 and i <=604 ):
           colors[i] =[0.1, 0.2, 0.2]
...
data=read('Pt10_3O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj@:')
