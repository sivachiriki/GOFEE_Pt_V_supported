...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 308 and i <=326 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 471 and i <=489 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 634 and i <=652):
           colors[i] =[0.1, 0.2, 0.2]
...
data=read('Pt10_9O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt10_9O2_Al2O3_0001_DFTopt_g{}'.format(j)
