...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 290 and i <=301 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 441 and i <=452 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 593 and i <=604 ):
           colors[i] =[0.1, 0.2, 0.2]
...
data=read('Pt7_4O2_Al2O3_KRRfund9l_DFTrelaxedsorted1.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt7_4O2_Al2O3_0001_DFTopt_n{}'.format(j)
