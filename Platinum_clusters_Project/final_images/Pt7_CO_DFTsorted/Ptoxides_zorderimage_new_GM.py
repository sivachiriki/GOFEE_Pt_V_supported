...
for j in range(0,1):
    ...
    for i,a in enumerate(atoms):
        if i ==143:
            atoms.positions[i,1] =atoms.positions[i,1]+12.429
            atoms.positions[i,0] =atoms.positions[i,0]-7.176
        if i ==142:
            atoms.positions[i,1] =atoms.positions[i,1]+12.429
            atoms.positions[i,0] =atoms.positions[i,0]-7.176
    data[j] = atoms
   ...
   # write('newimage.traj',atoms)
    ...
    ax.set_xlim([centreofmass[0]-8.0, centreofmass[0]+8.0])
    ax.set_ylim([centreofmass[1]-8.50+0.5, centreofmass[1]+8.50-1.50])
