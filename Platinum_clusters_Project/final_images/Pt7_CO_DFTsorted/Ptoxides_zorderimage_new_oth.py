...
for j in range(0,1):
    ...
    del atoms[atoms.positions[:,1] <= centreofmass[1]-7.30]
  
    colorlenth = len(atoms) 
    #view(atoms)
    plt.figure(figsize=(4.0,6.0))
    gs = gridspec.GridSpec(2, 1,
                           height_ratios=[4,6.35])
    ...
    ax.set_xlim([centreofmass[0]-7.0, centreofmass[0]+7.50])
    ...
    ax.set_xlim([centreofmass[0]-7.0, centreofmass[0]+7.50])
    ax.set_ylim([centreofmass[1]-6.5, centreofmass[1]+8.30])
