...
traj = Trajectory('Pt10_6O2_Al2O3_KRRfund9l_DFTrelaxed.traj','w')
for i in range(1,10):
    data =[]
    stru = '../Pt10_6O2_{}/Pt10_6O2_Al2O3_KRRfund9l_DFTrelaxed.traj'.format(i)
    data = read(stru+'@:')
    data.sort(key=lambda x: x.get_potential_energy())
    for j in range(0,len(data)):
        traj.write(data[j])

data1=read('Pt10_6O2_Al2O3_KRRfund9l_DFTrelaxed.traj@:')
data1.sort(key=lambda x: x.get_potential_energy())
traj = Trajectory('Pt10_6O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj','w')
