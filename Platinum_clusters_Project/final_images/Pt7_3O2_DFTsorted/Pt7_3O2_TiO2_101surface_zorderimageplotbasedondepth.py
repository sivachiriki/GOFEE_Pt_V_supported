...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 290 and i <=295 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 438 and i <=443 ):
           colors[i] =[0.1, 0.2, 0.2]
        if (atom.number ==8 and i >= 586 and i <=591 ):
           colors[i] =[0.1, 0.2, 0.2]
...
data=read('Pt7_3O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj@:')
...
for j in range(len(data)):
    ...
    plt.tight_layout()
    name ='Pt7_3O2_Al2O3_0001_DFTopt_g{}'.format(j)
