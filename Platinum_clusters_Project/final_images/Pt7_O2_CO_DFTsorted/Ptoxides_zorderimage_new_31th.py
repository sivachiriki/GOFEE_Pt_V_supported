...
for j in range(31,len(data)):
   ...
   # write('newimage.traj',atoms)
    ...
    del atoms[atoms.positions[:,0] <= centreofmass[0]-9.0]
    del atoms[atoms.positions[:,1] >= centreofmass[1]+9.50]
    del atoms[atoms.positions[:,1] <= centreofmass[1]-6.2]
    ...
    ax.set_xlim([centreofmass[0]-8.0, centreofmass[0]+8.0])
    ax.set_ylim([centreofmass[1]-5.5, centreofmass[1]+9.0])
