...
traj = Trajectory('Pt13_6O2_Al2O3_KRRfund9l_DFTrelaxed.traj','w')
for i in range(1,9):
    data =[]
    stru = '../Pt13_6O2_{}/Pt13_6O2_Al2O3_KRRfund9l_DFTrelaxed.traj'.format(i)
    data = read(stru+'@:')
    data.sort(key=lambda x: x.get_potential_energy())
    for j in range(0,len(data)):
        traj.write(data[j])

data1=read('Pt13_6O2_Al2O3_KRRfund9l_DFTrelaxed.traj@:')
data1.sort(key=lambda x: x.get_potential_energy())
traj = Trajectory('Pt13_6O2_Al2O3_KRRfund9l_DFTrelaxedsorted.traj','w')
