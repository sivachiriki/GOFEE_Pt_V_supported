...
for j in range(0,len(data)):
   ...
   # write('newimage.traj',atoms)
    ...
    img = atoms.copy()
    if (j!=1 and j!=3 and j!=6 and j!=7):
...
for j in range(0,14):
   ...
   # write('newimage.traj',atoms)
    a=atoms
    del atoms[[atom.index for atom in atoms if atom.index <=colorlenth*5-24 or atom.index >=colorlenth*5]]
    #view(atoms)
    centreofmass = a.get_center_of_mass()
    atoms = data[j]*(3,3,1)
    a=atoms
...
fig.text(0.5, 0.89, 'Lowest Isomers of Pt$_7$O$_{14}$ and Pt$_7$O$_{14}$CO', ha='center',fontsize=14)
name = sys.argv[3]
name =name
