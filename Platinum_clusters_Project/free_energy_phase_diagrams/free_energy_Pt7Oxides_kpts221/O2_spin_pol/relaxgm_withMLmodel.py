...
traj = Trajectory('O2_molecule_DFTrelaxed.traj','w')
data = read('O2_molecule.traj@:')
calc = GPAW(mode=PW(500),xc='PBE',
            spinpol=True,
            basis='dzp')
for i in range(0,len(data)):
    name ='O2_molecule_{}'.format(i)
    a=data[i]
    a.set_initial_magnetic_moments(magmoms=[3.0,3.0])
    a.set_calculator(calc)
    a_relaxed = relaxGPAW(a, name, forcemax=0.025, niter_max=3,steps=100)
