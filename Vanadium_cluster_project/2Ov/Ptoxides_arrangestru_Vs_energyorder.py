...
ax2.yaxis.set_ticks(np.arange(-1.60, -1.10, 0.1))
#----------------------------------------------------------------------------------------#
...
for j in range(0,len(data)):
    ...
    print(dy, xy)
    ax = plt.axes([xy[0], xy[1]-(dy)/30.50, 0.120, 0.120])
    img = atoms.copy()
   ...
   # ax = plt.subplot()
    ...
    xy = transform((j+0.5, energydif[j]))
    print(dy, xy) 
    ax = plt.axes([xy[0], xy[1]-(dy)/3.40, 0.120, 0.120])
    cell = atoms.get_cell()
    img = atoms.copy()
...
for j in range(0,len(data)):
    ...
    xy = transform((j+0.5, energydif[j]))
    print(dy, xy)
    ax = plt.axes([xy[0], xy[1]-(dy)/15.50, 0.120, 0.120])
    img = atoms.copy()
    plot_conf(ax, img,colorlenth)
   ...
   # ax = plt.subplot()
    ...
    xy = transform((j+0.5, energydif[j]))
    print(dy, xy)
    ax = plt.axes([xy[0], xy[1]-(dy)/3.15, 0.120, 0.120])
    cell = atoms.get_cell()
    img = atoms.copy()
    plot_conf(ax, img,colorlenth, rot=True)
