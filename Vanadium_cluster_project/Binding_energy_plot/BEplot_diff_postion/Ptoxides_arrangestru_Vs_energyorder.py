...
fig, [[ax1,ax3],[ax2,ax4]] = plt.subplots(nrows=2, ncols=2,figsize=(10.0,10.50))
for i in range(0,7):
    ax1.plot(i,0.0)
    ax2.plot(i,-1.30)
    ax3.plot(i,0.0)
...
ax3.set_xlim(0.4, 3.5)
ax4.set_xlim(0.4, 3.5)
ax1.yaxis.set_ticks(np.arange(-1.0, 0.3, 0.2))
ax2.yaxis.set_ticks(np.arange(-1.80, -1.0, 0.2))
ax3.yaxis.set_ticks(np.arange(-1.0, 0.3, 0.2))
ax4.yaxis.set_ticks(np.arange(-1.80, -1.0, 0.2))
#----------------------------------------------------------------------------------------#
...
for j in range(0,len(data)):
    ...
    xy = transform((j+0.5, energydif[j]))
    print(dy, xy)
    ax = plt.axes([xy[0], xy[1]+(dy)/6.0, 0.120, 0.120])
    img = atoms.copy()
    plot_conf(ax, img,colorlenth)
