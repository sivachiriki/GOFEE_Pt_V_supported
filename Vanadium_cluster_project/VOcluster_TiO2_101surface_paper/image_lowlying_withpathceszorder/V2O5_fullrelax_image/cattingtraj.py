...
traj = Trajectory('V2O5_101_fullyrelax_DFTrelaxed.traj','w')
for i in range(1,3):
    data =[]
    stru = '{}.traj'.format(i)
    data = read(stru+'@:')
...
data1=read('V2O5_101_fullyrelax_DFTrelaxed.traj@:')
data1.sort(key=lambda x: x.get_potential_energy())
traj = Trajectory('V2O5_TiO2_101sur_DFTlowlyingsorted.traj','w')
