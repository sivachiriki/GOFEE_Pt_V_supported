...
fig, [ax1,ax2] = plt.subplots(nrows=2, ncols=1,figsize=(8.0,9.50))
for i in range(0,4):
    ax1.plot(i,0.0)
    ax2.plot(i,-1.30)
#---------------------------------------------------------------------------------------#
...
ax2.xaxis.set_ticks([1,2,3])
ax1.yaxis.set_ticks(np.arange(-1.0, 0.5, 0.2))
ax2.yaxis.set_ticks(np.arange(-1.90, -1.10, 0.2))
#----------------------------------------------------------------------------------------#
ax1.spines['bottom'].set_visible(False)
ax2.spines['top'].set_visible(False)
fig.text(0.5, 0.04, 'Lowest Isomers found for V$_2$O$_5$, V$_2$O$_5$H$_2$O and V$_2$O$_5$2H$_2$O', ha='center',fontsize=12)
fig.text(0.01, 0.5, 'Binding Energy of Isomers (eV)', va='center', rotation='vertical',fontsize=12)
...
for j in range(0,len(data)):
    ...
    xy = transform((j+0.8, energydif[j]))
    print(dy, xy)
    ax = plt.axes([xy[0], xy[1]-(dy)/3.50, 0.170, 0.170])
    img = atoms.copy()
    plot_conf(ax, img,colorlenth)
   ...
   # ax = plt.subplot()
    ...
    xy = transform((j+0.8, energydif[j]))
    print(dy, xy)
    ax = plt.axes([xy[0], xy[1]-(dy)/1.30, 0.170, 0.170])
    cell = atoms.get_cell()
    img = atoms.copy()
    plot_conf(ax, img,colorlenth, rot=True)
