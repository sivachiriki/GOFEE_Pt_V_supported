...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if (atom.number ==8 and i >= 108 and i <= 112):
...
data=read('V2O4_TiO2_101_1by3cell_Ov_U4.0DFTrelaxedsorted.traj@:')

#for j in range(len(data)):
for j in range(3,4):
    image = data[j]
    for i,a in enumerate(image):
        if a.position[1] >11.200:
    ...
    plt.tight_layout()
    name = 'V2O4_TiO2_101_1by3cell_Ov_DFTcsr_g{}'.format(j)
