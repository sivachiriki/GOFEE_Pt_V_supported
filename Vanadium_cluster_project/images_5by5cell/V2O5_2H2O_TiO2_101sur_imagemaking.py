...
def plot_conf(ax, atoms, rot=False):
    ...
    for i, atom in enumerate(atoms):
        if (atom.number ==23):
           ...
           colors[i] =[255/255, 255/255, 51/255]
           #colors[i] = #8B0000
        if (atom.number ==1):
           #colors[i] =[0.9, 0.3, 0.9]
           colors[i] =[0/255, 0/255, 255/255]
           #colors[i] = #8B0000
        if (atom.number ==8 and i >= 648 ):
           #colors[i] =[0.1, 0.2, 0.5]
           colors[i] =[255/255, 51/255, 153/255]
           #colors[i] = #8B0000
...
data=read('V2O5_TiO2_101_3by6cell_GMDFTrelaxed.traj@:')
...
for j in range(len(data)):
    ...
    ax.set_xlim([7.0, 27.8])
    ax.set_ylim([12.7, 20.0])
    ax.set_yticks([])
    ax.set_xticks([])
    ax.set(aspect=1)
    ...
    plt.tight_layout()
    name ='V2O5_TiO2_101sur_cherry{}'.format(j)
