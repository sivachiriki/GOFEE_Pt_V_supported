...
for j in range(len(data)):
    ...
    for i,a in enumerate(image):
        if a.position[1] >17.10:
           image.positions[i,1] =image.positions[i,1]-22.934
    for i,a in enumerate(image):
        if a.position[0] >15.70:
           image.positions[i,0] =image.positions[i,0]-20.683
  
    #view(image) 
    # Make array of indices for atoms that should be repeated in x and y directions                                       
    plt.figure(figsize=(6.30,8.0))
    
    gs = gridspec.GridSpec(2, 1,
                           height_ratios=[5.27,13.0])
